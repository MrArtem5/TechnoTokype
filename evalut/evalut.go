...
func Eval(node ast.Node, env *Environment) value.Value {
	returnFlag = false
	returnValue = value.NewNilValue()
	return evalInternal(node, env)
}

func evalInternal(node ast.Node, env *Environment) (result value.Value) {
	switch node := node.(type) {
	case *ast.Program:
		var result value.Value = value.NewNilValue()
		for _, statement := range node.Statements {
			if _, ok := statement.(*ast.FunctionStatement); ok {
				evalInternal(statement, env)
				if returnFlag {
					return returnValue
				}
			}
		...
		for _, statement := range node.Statements {
			if _, ok := statement.(*ast.FunctionStatement); !ok {
				result = evalInternal(statement, env)
				if returnFlag {
					return returnValue
				}
			}
	...
	case *ast.ReturnStatement:
		...
		if node.Value != nil {
			val = evalInternal(node.Value, env)
			if returnFlag {
				return returnValue
			}
		}
	...
	case *ast.MapLiteral:
		...
		for keyNode, valueNode := range node.Pairs {
			key := evalInternal(keyNode, env)
			if returnFlag {
				return returnValue
			}
			val := evalInternal(valueNode, env)
			if returnFlag {
				return returnValue
			}
			pairs[key] = val
	...
	case *ast.InfixExpression:
		leftVal := evalInternal(node.Left, env)
		if returnFlag {
			return returnValue
		}
		rightVal := evalInternal(node.Right, env)
		if returnFlag {
			return returnValue
		}
	...
	case *ast.ForStatement:
		evalInternal(node.Initialization, env)
		if returnFlag {
			return returnValue
		}

		for {
			cond := evalInternal(node.Condition, env)
			if returnFlag {
				return returnValue
			}
			if cond.IsNil() {
			...
			evalInternal(node.Body, env)
			if returnFlag {
				return returnValue
			}
			evalInternal(node.Update, env)
			if returnFlag {
				return returnValue
			}
		}
	...
	case *ast.IndexExpression:
		left := evalInternal(node.Left, env)
		if returnFlag {
			return returnValue
		}
		if left.IsNil() {
		...
		index := evalInternal(node.Index, env)
		if returnFlag {
			return returnValue
		}
		if index.IsNil() {
	...
	case *ast.AssignIndexStatement:
		left := evalInternal(node.Left.Left, env)
		if returnFlag {
			return returnValue
		}
		if left.IsNil() {
		...
		index := evalInternal(node.Left.Index, env)
		if returnFlag {
			return returnValue
		}
		if index.IsNil() {
	...
	case *ast.SliceExpression:
		...
		if node.Start != nil {
			startVal := evalInternal(node.Start, env)
			if returnFlag {
				return returnValue
			}
			if startVal.IsNil() {
		...
		if node.End != nil {
			endVal := evalInternal(node.End, env)
			if returnFlag {
				return returnValue
			}
			if endVal.IsNil() {
	...
	case *ast.IfExpression:
		condition := evalInternal(node.Condition, env)
		if returnFlag {
			return returnValue
		}
		if condition.IsNil() {
		...
		if condition.Type == value.ValBool && condition.Bool {
			return evalInternal(node.Consequence, env)
		}

		for _, elif := range node.Alternative {
			elifCond := evalInternal(elif.Condition, env)
			if returnFlag {
				return returnValue
			}
			if elifCond.IsNil() {
			...
			if elifCond.Type == value.ValBool && elifCond.Bool {
				return evalInternal(elif.Consequence, env)
			}
		}

		if node.Else != nil {
			return evalInternal(node.Else, env)
		}
	...
	case *ast.CallExpression:
		...
		case *ast.Identifier:
			...
			if funcName == "repeat" {
				...
				val := evalInternal(node.Arguments[0], env)
				if returnFlag {
					return returnValue
				}
				countVal := evalInternal(node.Arguments[1], env)
				if returnFlag {
					return returnValue
				}
			...
			if funcName == "reserve" {
				...
				listArg := evalInternal(node.Arguments[0], env)
				if returnFlag {
					return returnValue
				}
				...
				capacityVal := evalInternal(node.Arguments[1], env)
				if returnFlag {
					return returnValue
				}
			...
			if funcName == "input" {
				if len(node.Arguments) > 0 {
					prompt := evalInternal(node.Arguments[0], env)
					if returnFlag {
						return returnValue
					}
					if !prompt.IsNil() {
			...
			if funcName == "negate" {
				...
				arg := evalInternal(node.Arguments[0], env)
				if returnFlag {
					return returnValue
				}
				if arg.IsNil() {
			...
			if funcName == "push" {
				...
				elemArg := evalInternal(node.Arguments[1], env)
				if returnFlag {
					return returnValue
				}
		...
		default:
			fnObjVal := evalInternal(node.Function, env)
			if returnFlag {
				return returnValue
			}
			if fnObjVal.IsNil() {
		...
		return evalInternal(fnObj.Body.(*ast.BlockStatement), extendedEnv)
	}
